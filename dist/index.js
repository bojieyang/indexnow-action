...
const styles = __nccwpck_require__(2068);

const {verify, calculateWith} = __nccwpck_require__(1252);
...
async function prepareForSubmit(options) {
  try {
    ...
    if (urls.length > options.limit) {
      core.notice(
        'The number of candidate urls exceeds the upper limit, will be truncated. The Number of candidate links is ${sites.length}. The limit set in inputs is ${options.limit}.'
      );
...
function getSitemapLocation() {
  ...
  throw new Error(
    `sitemap-location with value ${sitemapInput} is invalid format.`
  );
}
...
function getLimit() {
  ...
  if (limit > INDEXNOW_UPPER_LIMIT) {
    throw new Error(
      `limit with the value ${limit} exceeds upper limit. the upper limit is ${INDEXNOW_UPPER_LIMIT}.`
    );
  }
  return limit;
}
...
function getFailureStrategy() {
  ...
  if (strategy === undefined) {
    throw new Error(
      `failure-strategy with the value ${strategyInput} is not available.`
    );
  }
  return strategy;
}
